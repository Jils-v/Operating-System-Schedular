...
public class PRIORITY extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        pr_answer = (TextView)findViewById(R.id.priority_answer);
    ...
    public void click(View v)
    {
        ...
        if(n.matches("[0-9]+") && n.length()>0)
        {
            ...
            if(n1 > 10)
            {
                Toast.makeText(PRIORITY.this, "Number Of Process is greater than 10", Toast.LENGTH_SHORT).show();
            }
            else {
                    String[] temp1 = test_a.split(" ");
                    String[] temp2 = test_b.split(" ");
                    String[] temp3 = test_p.split(" ");

                    test_a = test_a.replaceAll(" ", "");
                    test_b = test_b.replaceAll(" ", "");
                    test_p = test_p.replaceAll(" ", "");

                    if (test_a.matches("[0-9]+") && temp1.length == n1) {
                        if (test_b.matches("[0-9]+") && temp2.length == n1) {
                            if(test_p.matches("[0-9]+") && temp3.length == n1) {
                                String s = a + " " + b;
                                String[] p = pr.split(" ");
                                String[] str = null;
                                str = s.split(" ");
                                ...
                                PyObject pyobj = py.getModule("PRIORITY_scheduling");

                                PyObject pyObject = pyobj.callAttr("algorithm", n, p, str);
                                ...
                                pr_answer.setText(ans[6]);

                            }
                            else
                            {
                                Toast.makeText(PRIORITY.this, "invalid input", Toast.LENGTH_SHORT).show();
                            }
