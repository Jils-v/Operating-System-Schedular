...
public class RR extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        t = (EditText)findViewById((R.id.priority));


    }
    public void click(View v)
    {
        String ttq = t.getText().toString().trim();
        String n = nop.getText().toString().trim();
        String a = at.getText().toString().trim();
        String b = bt.getText().toString().trim();

        if(n.matches("[0-9]+") && n.length()>0 && ttq.matches("[0-9]+") && ttq.length()>0)
        {
            String test_a = a;
            String test_b = b;

            int n1 = Integer.parseInt(n);
            int tq = Integer.parseInt(ttq);

            if(n1 > 10)
            {
                Toast.makeText(RR.this, "Number Of Process is greater than 10", Toast.LENGTH_SHORT).show();
            }
            else {
                if (tq > 100) {
                    Toast.makeText(RR.this, "Time Quontum is very large", Toast.LENGTH_SHORT).show();
                } else {
                    ...
                    if (test_a.matches("[0-9]+") && temp1.length == n1) {
                        if (test_b.matches("[0-9]+") && temp2.length == n1) {
                            ...
                            PyObject pyobj = py.getModule("RR");

                            PyObject pyObject = pyobj.callAttr("algorithm", n, tq, str);
                            ...
                            Toast.makeText(RR.this, "invalid input", Toast.LENGTH_SHORT).show();
                        }
                    } else {
                        Toast.makeText(RR.this, "invalid input", Toast.LENGTH_SHORT).show();
                    }
