def algorithm(nop, a):
    ...
    while(len(at) != 0):
        ...
        for i in ready_queue:
            bt_of_ready.append(bt[i])
        if(len(bt_of_ready) == 0):
            cpu_time = cpu_time + 1
            continue
        lowest_bt = min(bt_of_ready)
        temp_index = bt.index(lowest_bt)
        ...
        counter = 0
        for element in final_bt:
            if(element == lowest_bt):
                counter = counter+1
        if(counter == 1):
            temp_index2 = final_bt.index(lowest_bt)
            index_manager.append(temp_index2)
        else:
            temp_index2 = temp_final_bt.index(lowest_bt)
            temp_final_bt[temp_index2] = " "
            index_manager.append(temp_index2)
